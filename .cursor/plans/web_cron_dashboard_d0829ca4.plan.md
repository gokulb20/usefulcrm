...
overview: Add a "Cron" virtual folder to the web app sidebar that reads `~/.hermes/cron/` data, showing all cron jobs, their heartbeat/scheduling status, run history, and full session transcripts with thinking for each run.
todos:
  - id: types
...
- **Job definitions**: `~/.hermes/cron/jobs.json` (`CronStoreFile` with `CronJob[]`)
- **Run logs per job**: `~/.hermes/cron/runs/{jobId}.jsonl` (`CronRunLogEntry` per line)
- **Run session transcripts**: `~/.hermes/agents/{agentId}/sessions/{sessionId}.jsonl`
...
flowchart LR
  subgraph disk [Disk Storage]
    JobsJSON["~/.hermes/cron/jobs.json"]
    RunLogs["~/.hermes/cron/runs/{jobId}.jsonl"]
    Sessions["~/.hermes/agents/{agentId}/sessions/{sessionId}.jsonl"]
  end
...
Read `~/.hermes/cron/jobs.json` and return all jobs with computed status fields. Also scan `~/.hermes/cron/runs/` to get latest run info. Include heartbeat info by reading agent config from `~/.hermes/config.yaml` (heartbeat interval, next due estimate).
...
Read `~/.hermes/cron/runs/{jobId}.jsonl` using the same logic as `readCronRunLogEntries` in [src/cron/run-log.ts](src/cron/run-log.ts). Return entries with `?limit=N` support.
