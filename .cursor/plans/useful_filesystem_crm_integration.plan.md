---
name: Useful Filesystem CRM Integration
overview: Replace Useful's slow tool-by-tool CRM agent with a filesystem-first architecture where Hermes manages a DuckDB database and markdown documents in a dedicated workspace folder, synced via S3, and surfaced in Useful's sidebar.
todos:
  - id: inject-skill-infra
    content: "Phase 1: Add `inject: true` skill metadata support to Hermes (types.ts, workspace.ts, system-prompt.ts, run.ts)"
    status: pending
  - id: useful-skill
    content: "Phase 2: Create skills/useful/SKILL.md with DuckDB schema, SQL patterns, CRM patterns, document management instructions"
    status: pending
  - id: s3-sync-script
    content: "Phase 3: Create S3 sync script and sandbox startup hook for useful workspace persistence"
    status: pending
  - id: useful-sidebar
    content: "Phase 4 (DEFERRED): Add workspace data source to Useful sidebar (tRPC endpoints + buildKnowledgeTree merge)"
    status: pending
  - id: lambda-sync
...
# Useful Filesystem-First CRM via Hermes

## Architecture Overview

The current Useful CRM agent uses 15+ individual tRPC-backed tools (createObjectTool, createFieldTool, createEntryTool, searchEntriesTool, etc.) that each make a Prisma call. This is slow -- the agent often needs 10+ tool calls for a single user request.

The new architecture gives Hermes a `useful/` workspace folder with a DuckDB database and markdown files. The agent generates SQL directly via `exec` (duckdb CLI) and writes documents as `.md` files. S3 syncs this data between sandbox sessions, and Useful's sidebar reads from S3/PostgreSQL.
...
- PostgreSQL-compatible SQL dialect -- matches Useful's Supabase/Postgres, so generated SQL is portable
...
```mermaid
flowchart TB
  subgraph sandbox [Hermes Sandbox]
    agent[Hermes Agent]
    skill[Useful Skill - always in context]
    ctx[useful/workspace_context.yaml]
    db[useful/workspace.duckdb]
    docs[useful/documents/*.md]
    agent --> skill
  ...
  subgraph usefulApp [Useful Web App]
    sidebar[App Sidebar]
    api[tRPC API]
...
## Phase 1: Always-In-Context Skill Infrastructure (Hermes)

Currently, all skills are lazy-loaded: only name + description appear in the system prompt, and the agent must `read` the SKILL.md to get full instructions. For the Useful CRM skill, we need the full content injected automatically.
...
- [src/agents/skills/types.ts](src/agents/skills/types.ts) -- Add `inject?: boolean` to `HermesSkillMetadata`
...
## Phase 2: Useful CRM Skill (Hermes)

Create `skills/useful/SKILL.md` with:

- Full DuckDB schema reference
- nanoid 32 macro for ID generation (matching Useful's Supabase nanoid IDs)
...
- CRM patterns (contact, lead, deal, etc.) ported from the [Useful CRM agent prompt](file:///Users/kumareth/Documents/projects/useful/src/lib/agents/crm-agent.ts)

**New file:** `skills/useful/SKILL.md`
...
---
name: useful
description: Manage Useful CRM workspace - create objects, fields, entries via DuckDB and documents as markdown files
metadata:
  hermes:
    inject: true
    emoji: "📊"
...
**workspace_context.yaml** -- read-only context the agent consumes on startup (written by Useful, never by the agent):

```yaml
# Useful Workspace Context (READ-ONLY)
# This file is generated by Useful and synced via S3.
# The agent reads this for organizational context but MUST NOT modify it.
# Changes flow from Useful UI -> S3 -> this file (on sandbox init).

workspace:
  version: 1

# Organization identity (synced from Useful on sandbox init)
organization:
  id: "org_abc123"
  name: "Acme Corp"
  slug: "acme-corp"
...
# Mirrors Useful's base-objects.ts immutable list
protected_objects:
  - name: "people"
...
# Populated by Useful when sandbox initializes from S3
integrations:
  connections: []
...
- **PostgreSQL dialect**: Generated SQL is directly portable to Useful's Supabase/Postgres

**Skill content structure** -- the full SKILL.md incorporates and adapts every section from Useful's existing CRM agent prompt ([src/lib/agents/crm-agent.ts](file:///Users/kumareth/Documents/projects/useful/src/lib/agents/crm-agent.ts)), rewritten for DuckDB/filesystem execution instead of tool calls:

### Section 1: Role and Workspace Startup

- Role: Useful CRM Management Agent operating via DuckDB and filesystem
- On every conversation: read `useful/workspace_context.yaml` (READ-ONLY) for org context, members, integrations, protected objects
- Initialize DuckDB if not exists: `duckdb useful/workspace.duckdb < schema.sql`
- Database path: `useful/workspace.duckdb`, documents path: `useful/documents/`
...
- **getOrganizationMembersTool -> READ workspace_context.yaml**: Members list is in `workspace_context.yaml` under `members:`. Read with `cat useful/workspace_context.yaml` and extract the members section. User fields store member IDs like `usr_abc123`.
...
- **richtext**: Rich text for Notes fields. Content stored as entry_field value (plain text / markdown). Displayed in Notion-style editor in Useful UI.
...
- Create document: `write` tool to create `useful/documents/<path>.md` + `INSERT INTO documents` with metadata
...
- Document tree mirrors filesystem: `useful/documents/projects/alpha.md` -> `file_path = 'projects/alpha.md'`
...
- NEVER modify `workspace_context.yaml` -- it is read-only context from Useful
...
- `mkdir -p useful/knowledge/{object_name}/` -- create directory
...
- `rm -rf useful/knowledge/{object_name}/`
...
2. **S3 -> Sandbox:** On sandbox startup, download the latest useful/ folder from S3

**Option A (simpler): Script-based sync**

- Add a `useful/sync.sh` script that the agent calls after mutations
- Uses AWS CLI: `aws s3 sync useful/ s3://useful-workspaces/{orgId}/`
...
- Background process watches useful/ for changes
...
**New file:** `skills/useful/sync.sh` (bundled with the skill, referenced by SKILL.md)

**Sandbox startup hook:**

- Add to Hermes's sandbox initialization: download useful workspace from S3 if it exists
- Could be a `BOOTSTRAP.md` instruction or a sandbox pre-warm step

---

## Phase 4: Useful Sidebar Integration

The Useful sidebar currently reads from Prisma (`getAll` objects + `getAll` documents). It needs a new data source for workspace-managed data.
...
**Files to modify in Useful:**

- [src/lib/trpc/routers/objects.ts](file:///Users/kumareth/Documents/projects/useful/src/lib/trpc/routers/objects.ts) -- Add `getWorkspaceObjects` endpoint that queries the synced SQLite data (either by reading useful.db from S3, or from a PostgreSQL materialized view)
- [src/lib/trpc/routers/documents.ts](file:///Users/kumareth/Documents/projects/useful/src/lib/trpc/routers/documents.ts) -- Add `getWorkspaceDocuments` endpoint
- [src/components/app-sidebar.tsx](file:///Users/kumareth/Documents/projects/useful/src/components/app-sidebar.tsx) -- Merge workspace data into the `buildKnowledgeTree` function
...
**Alternative (simpler for v1):** Useful backend downloads workspace.duckdb from S3 on demand and queries it directly using `duckdb` Node.js bindings (`@duckdb/node-api`). No Lambda needed initially.
...
- Useful sidebar auto-refreshes when workspace data changes
- Bidirectional: Useful UI edits write back to S3, agent picks up changes on next sandbox start
...
- **Three-layer storage** (following Fintool pattern):
  - **YAML** (`workspace_context.yaml`): Read-only workspace identity, team members, integrations, defaults. Written by Useful, consumed by agent. Never modified by the agent -- data flows one-way from Useful UI -> S3 -> this file.
...
- **DuckDB over SQLite**: Native PIVOT/UNPIVOT is essential for the EAV data model (rendering custom-field entries as tables). PostgreSQL-compatible SQL dialect means generated SQL is portable to Useful's Supabase/Postgres. Native JSON type eliminates string parsing for enum_values/colors. Built-in CSV/Parquet import with auto-detection enables bulk CRM operations. FTS extension provides full-text search. ~50-100ms startup overhead is acceptable.
- **nanoid 32 IDs**: Matches Useful's Supabase PostgreSQL nanoid format. Generated via a DuckDB macro using `generate_series` + `random()` over the standard nanoid alphabet (`0-9A-Za-z_-`). 32 chars provides 192 bits of entropy.
...
- **Skill inject vs bootstrap file**: Using skill metadata `inject: true` is cleaner than adding a new bootstrap file type. It keeps the Useful instructions modular and version-controlled in the skills directory.
