...
overview: "Complete redesign of the Hermes web app (apps/web/) to match the Useful design system: switch from dark theme to light, adopt Instrument Serif + Inter fonts, port the Useful color palette and layout patterns, and rewrite every component and page from the ground up."
todos:
  - id: foundation
    content: "Phase 1: Rewrite globals.css (light theme, HSL tokens, font imports) and layout.tsx (next/font, remove dark mode)"
    status: pending
  - id: landing
    content: "Phase 2: Rewrite app/page.tsx as Useful-style landing page (navbar, hero, demo sections, footer)"
    status: pending
  - id: layout-shell
    content: "Phase 3: Create app-navbar.tsx, rewrite workspace/page.tsx layout with top navbar + sidebar grid"
    status: pending
  - id: sidebar
    content: "Phase 4: Redesign workspace-sidebar.tsx and file-manager-tree.tsx to match Useful sidebar"
    status: pending
  - id: data-table
    content: "Phase 5: Redesign object-table.tsx with Useful-style toolbar, sticky headers, pagination, enum badges"
    status: pending
  - id: kanban
...
# Full Web UI Redesign — Useful Design System

## Current State

The Hermes web app is a **dark-themed** Next.js 15 app with:
...
## Target State (Useful Design)

Per the screenshots and Useful source:
...
The Hermes app uses **Tailwind v4** (CSS-based config via `@import "tailwindcss"`), while Useful uses Tailwind v3 (JS config). We will keep Tailwind v4 but port all design tokens into `globals.css` using `@theme` blocks and CSS custom properties. No downgrade needed.

## Architecture Decision: Light + Dark Theme

Useful is light-only. We will use Useful's light palette as the `:root` default AND create a custom dark palette under `.dark` (class-based toggle via `<html class="dark">`). All components will use CSS variable references (e.g. `bg-background`, `text-foreground`, `border-border`) so they automatically adapt. No hardcoded hex/rgb in components.

**Light palette** (from Useful):
...
**Dark palette** (custom, designed to complement Useful's light theme):
...
- `:root` block: Useful's light-theme HSL palette (background, foreground, card, primary, secondary, muted, accent, destructive, border, ring, sidebar, chart-1 through chart-5)
...
- Update metadata title/description to "Useful" branding
...
**[app/page.tsx](apps/web/app/page.tsx)** — Full rewrite to match Useful landing:

- Sticky navigation bar (logo "Useful" in `font-lora`, Login button in rounded-full blue pill)
...
- Add top `AppNavbar` component: `bg-neutral-100 border-b border-border shadow-[0_0_40px_rgba(0,0,0,0.05)]`
  - Left: org logo + "Powered by Useful" + org name in `font-instrument`
...
**[app/components/workspace/object-table.tsx](apps/web/app/components/workspace/object-table.tsx)** — Complete rewrite to match Useful data-table:
...
- Enum badges: colored pill style matching Useful (translucent background + border)
...
**Prompt templates** (simplified set for Hermes):

- Follow-up Emails, Calendly Prep, Zoom Recap, Facebook Leads, Calendar Sync, Salesforce Sync, Intercom Chat (matching the Useful screenshot chips)
...
- Exceptions: Useful-specific decorative elements (landing page traffic-light dots, brand colors) can use fixed values
