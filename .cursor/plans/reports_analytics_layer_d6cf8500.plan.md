...
overview: "Add a generative-UI reports feature to the Useful web app: the agent creates JSON report definitions with SQL queries and chart configs, rendered live via Recharts in both the workspace view and inline in chat as artifacts."
todos:
  - id: recharts-dep
  ...
  - id: sidebar-reports
    ...
    status: completed
  - id: skill-update
    content: Add Section 13 (Report Generation) to skills/useful/SKILL.md with schema, examples, and agent instructions
    status: completed
isProject: false
---

# Reports / Analytics Layer for Useful Workspace
...
Stored as `.report.json` files in `useful/reports/` (or nested under any knowledge path). Agent generates these via the `write` tool.
...
## Phase 5: Useful Skill Updates

**Modify [skills/useful/SKILL.md](skills/useful/SKILL.md):**
...
| Modify | `skills/useful/SKILL.md` (report generation instructions)             |
`````
