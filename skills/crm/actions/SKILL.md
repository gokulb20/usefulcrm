...
Short JavaScript snippets stored directly in the action config. Executed server-side via `node` with the Useful SDK pre-loaded:
...
```json
{
  ...
  "variant": "success",
  "script": "await useful.objects.update(context.objectName, context.entryId, { Status: 'Contacted' });\nreturn { message: 'Updated' };",
  "runtime": "inline"
}
```
...
File-based scripts do NOT have the `useful` SDK. Use env vars + CLI/HTTP instead:
...
**The `useful` global and `context` object are ONLY available for inline scripts (`runtime: "inline"`).** File-based scripts do NOT have access to the SDK — they must use environment variables and raw HTTP/CLI calls (see [Environment Variables](#environment-variables) and [File-Based Script Patterns](#file-based-script-patterns) below).

For inline JS actions, the `useful` global provides:

| Method | Description |
|--------|-------------|
| `useful.objects.get(name, id)` | Get entry by object name and ID |
| `useful.objects.list(name)` | List entries for an object |
| `useful.objects.create(name, fields)` | Create a new entry |
| `useful.objects.update(name, id, fields)` | Update entry fields |
| `useful.objects.delete(name, id)` | Delete an entry |
| `useful.objects.bulkDelete(name, ids)` | Bulk delete entries |
| `useful.db.query(sql)` | Run read-only SQL on workspace DB |
| `useful.db.execute(sql)` | Execute SQL (write operations) |
| `useful.files.read(path)` | Read a workspace file |
| `useful.files.write(path, content)` | Write a workspace file |
| `useful.http.fetch(url, opts)` | Standard fetch (no proxy) |
| `useful.exec(cmd)` | Run a shell command synchronously |
| `useful.progress(percent, message)` | Report progress to UI |
| `useful.log(message, level)` | Log a message |
| `useful.complete(data)` | Signal successful completion |
| `useful.fail(message)` | Signal failure and exit |
| `useful.env.*` | Access all DENCH_* env vars as properties |
...
```json
{
  ...
  "label": "Mark Done",
  "variant": "success",
  "script": "await useful.objects.update(context.objectName, context.entryId, { Status: 'Done' });\nreturn { message: 'Marked as done' };",
  "runtime": "inline",
  "successLabel": "Done!",
  "autoResetMs": 2000
}
...
// File-based scripts use env vars — the `useful` SDK is NOT available here.
...
**2. Using the `useful` SDK in file-based scripts.**
The `useful` global and `context` object are ONLY available for inline scripts (`runtime: "inline"`). File-based scripts must use `process.env.DENCH_*` variables and shell/HTTP calls. See [File-Based Script Patterns](#file-based-script-patterns).
