...
describe("ProfileSwitcher workspace delete action", () => {
        ...
        if (listFetchCount === 1) {
          return jsonResponse({
            ...
            workspaces: [
              {
                name: "work",
                stateDir: "/home/testuser/.hermes-work",
                workspaceDir: "/home/testuser/.hermes-work/workspace",
                isActive: true,
                hasConfig: true,
              },
            ],
  ...
  it("falls back to a real workspace when API returns stale active workspace", async () => {
      ...
      if (url === "/api/workspace/list" && method === "GET") {
        return jsonResponse({
          ...
          workspaces: [
            {
              name: "ghost",
              stateDir: "/home/testuser/.hermes-useful",
              workspaceDir: null,
              isActive: true,
              hasConfig: true,
            },
            {
              name: "useful",
              stateDir: "/home/testuser/.hermes-useful",
              workspaceDir: "/home/testuser/.hermes-useful/workspace",
              isActive: false,
              hasConfig: true,
            },
          ],
    ...
    await waitFor(() => {
      const allUseful = screen.getAllByText("useful");
      expect(allUseful.length).toBeGreaterThanOrEqual(1);
