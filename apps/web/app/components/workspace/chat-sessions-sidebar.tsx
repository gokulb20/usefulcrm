...
export function ChatSessionsSidebar({
	...
	const [activeFilter, setActiveFilter] = useState("usefulcrm");
	...
	const usefulClawSessions = useMemo(
		() => sessions.filter((s) => !s.id.includes(":subagent:") && !s.filePath),
		[sessions],
	);

	const grouped = groupSessions(usefulClawSessions);
	...
	const tabs = useMemo(() => {
		const result: SidebarTab[] = [
			{ id: "usefulcrm", label: "UsefulCRM", count: usefulClawSessions.length },
		];
		...
		return result;
	}, [usefulClawSessions, gatewayByChannel, cronSessions, fileScopedSessions]);
	...
	const renderContent = () => {
		...
		if (activeFilter === "usefulcrm") {
			if (usefulClawSessions.length === 0) {
				return (
