...
type ContentState =
  ...
  | { kind: "app"; appPath: string; manifest: UsefulAppManifest; filename: string };

export type UsefulAppManifest = {
  name: string;
  description?: string;
  icon?: string;
  version?: string;
...
const STORAGE_LEFT = "useful-workspace-left-sidebar-width";
const STORAGE_RIGHT = "useful-workspace-right-sidebar-width";
const STORAGE_CHAT_SIDEBAR = "useful-workspace-chat-sidebar-width";
const STORAGE_ENTRY_PANEL = "useful-workspace-entry-panel-width";
...
function inferNodeTypeFromFileName(fileName: string): TreeNode["type"] {
  if (fileName.endsWith(".useful.app")) return "app";
...
function WorkspacePageInner() {
  ...
  const {
    ...
    browseDir, setBrowseDir, parentDir: browseParentDir, workspaceRoot, hermesDir,
    ...
    async (node: TreeNode) => {
      ...
      try {
        if (node.type === "object") {
          ...
          const manifestRes = await fetch(`/api/apps?app=${encodeURIComponent(node.path)}&file=.useful.yaml&meta=1`);
          let manifest: UsefulAppManifest = { name: node.name };
          if (manifestRes.ok) {
  ...
  const handleNodeSelect = useCallback(
    (node: TreeNode) => {
      // --- Browse-mode: detect special Hermes directories ---
      // When the user clicks a known Hermes folder while browsing the
      ...
      if (browseDir && isAbsolutePath(node.path)) {
        ...
        if (hermesDir) {
          ...
          if (hermesDir && node.path.startsWith(hermesDir + "/web-chat")) {
            setBrowseDir(null);
            setActivePath(null);
            setContent({ kind: "none" });
            openBlankChatTab();
    ...
    [loadContent, openBlankChatTab, openSessionChatTab, openTabForNode, cronJobs, browseDir, workspaceRoot, hermesDir, setBrowseDir],
  );
