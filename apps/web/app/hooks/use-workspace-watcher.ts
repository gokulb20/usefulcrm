...
export function useWorkspaceWatcher() {
  ...
  const [hermesDir, setOpenclawDir] = useState<string | null>(null);
  ...
  const fetchWorkspaceTree = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current && fetchVersionRef.current === version) {
        ...
        setOpenclawDir(data.hermesDir ?? null);
  ...
  return { tree, loading, exists, refresh, reconnect, browseDir, setBrowseDir, parentDir, workspaceRoot, hermesDir, activeWorkspace, showHidden, setShowHidden };
}
