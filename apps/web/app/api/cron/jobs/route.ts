...
import { resolveHermesStateDir } from "@/lib/workspace";

export const dynamic = "force-dynamic";

const CRON_DIR = join(resolveHermesStateDir(), "cron");
...
/** Read heartbeat config from ~/.hermes/config.yaml (best-effort). */
function readHeartbeatInfo(): { intervalMs: number; nextDueEstimateMs: number | null } {
  ...
  try {
    const agentsDir = join(resolveHermesStateDir(), "agents");
