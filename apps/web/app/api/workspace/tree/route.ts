...
/** Read .useful.yaml manifest from a .useful.app directory. */
async function readAppManifest(
  dirPath: string,
): Promise<TreeNode["appManifest"] | null> {
  const yamlPath = join(dirPath, ".useful.yaml");
  if (!await pathExists(yamlPath)) return null;
  ...
  try {
    ...
    const parsed = parseSimpleYaml(content);
    return {
      name: (parsed.name as string) || dirPath.split("/").pop()?.replace(/\.useful\.app$/, "") || "App",
...
async function buildTree(
  ...
  for (const { entry, absPath, effectiveType } of sorted) {
    ...
    if (effectiveType === "directory") {
      // Detect .useful.app folders -- treat as app nodes
      if (entry.name.endsWith(".useful.app")) {
        const manifest = await readAppManifest(absPath);
        const displayName = manifest?.name || entry.name.replace(/\.useful\.app$/, "");
...
export async function GET(req: Request) {
  ...
  const hermesDir = resolveHermesStateDir();
  ...
  if (!root) {
    ...
    return Response.json({ tree, exists: false, workspaceRoot: null, hermesDir, workspace });
  }
  ...
  return Response.json({ tree, exists: true, workspaceRoot: root, hermesDir, workspace });
}
