...
import { resolveHermesStateDir, resolveWebChatDir } from "./workspace";
...
const GLOBAL_KEY = "__hermes_subagentRuns" as const;
...
function legacySubagentEventsDir(): string {
	return join(resolveHermesStateDir(), "web-chat", "subagent-events");
}
...
function readDiskStatus(sessionKey: string): "running" | "completed" | "error" {
	...
	const registryPath = join(resolveHermesStateDir(), "subagents", "runs.json");
 ...
 * to the shared gateway registry (~/.hermes/subagents/runs.json).
