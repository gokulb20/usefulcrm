...
describeLive("subagent streaming (live)", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(tempDir, ".hermes");
    mkdirSync(path.join(tempDir, ".hermes"), { recursive: true });
    // Reset subagent singleton
    delete (globalThis as Record<string, unknown>)[
      "__hermes_subagentRuns"
    ];
  });
  ...
  it("persists subagent index to disk on registration", async () => {
    const webChatDir = path.join(tempDir, ".hermes", "web-chat");
