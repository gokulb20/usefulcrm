...
function readGatewayConfigFromStateDir(
	...
	const candidates = [join(stateDir, "hermes.json"), join(stateDir, "config.json")];
...
export function buildConnectParams(
	...
	return {
		...
		client: {
			...
			mode: clientMode,
			instanceId: "usefulcrm-web-server",
		},
		locale: "en-US",
		userAgent: "usefulcrm-web",
		role,
		scopes,
		caps,
		...(auth ? { auth } : {}),
 ...
 * this error means the device keypair at ~/.hermes-useful/identity/ is
 * missing or invalid.
 */
export function enhanceScopeError(raw: string): string | null {
	...
	return [
		...
		"Fix: run `npx usefulcrm bootstrap` to re-pair the device.",
	].join(" ");
}
...
export function parseErrorFromStderr(stderr: string): string | undefined {
	...
	for (const line of lines) {
		...
		if (/\b(error|failed|fatal)\b/i.test(trimmed)) {
			// Strip common prefixes like "[hermes]", timestamps, etc.
