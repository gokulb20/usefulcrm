...
describe("workspace (flat workspace model)", () => {
  ...
  const STATE_DIR = "/home/testuser/.hermes-useful";
  const UI_STATE_PATH = join(STATE_DIR, ".useful-ui-state.json");
  ...
  describe("getEffectiveProfile", () => {
    it("always returns 'useful' regardless of env/state (single profile enforcement)", async () => {
      ...
      expect(getEffectiveProfile()).toBe("useful");
    });
  });
  ...
  describe("discoverWorkspaces", () => {
    ...
    it("keeps root default and workspace-useful as distinct workspaces", async () => {
      const { discoverWorkspaces, mockReaddir, mockExists, mockReadFile } =
        await importWorkspace();
      mockReadFile.mockImplementation(() => {
        throw new Error("ENOENT");
      ...
      mockReaddir.mockReturnValue([
        makeDirent("workspace", true),
        makeDirent("workspace-useful", true),
      ] as unknown as Dirent[]);
      mockExists.mockImplementation((p) => {
        const s = String(p);
        return s === join(STATE_DIR, "workspace") || s === join(STATE_DIR, "workspace-useful");
      });
      ...
      expect(names).toContain("useful");
      const rootDefault = workspaces.find((workspace) => workspace.name === "default");
      const profileUseful = workspaces.find((workspace) => workspace.name === "useful");
      expect(rootDefault?.workspaceDir).toBe(join(STATE_DIR, "workspace"));
      expect(profileUseful?.workspaceDir).toBe(join(STATE_DIR, "workspace-useful"));
    });

    it("lists default, useful, and custom workspace side by side", async () => {
      const { discoverWorkspaces, mockReaddir, mockExists, mockReadFile } =
        await importWorkspace();
      mockReadFile.mockImplementation(() => {
        throw new Error("ENOENT");
      ...
      mockExists.mockImplementation((p) => {
        ...
        return (
          s === join(STATE_DIR, "workspace") ||
          s === join(STATE_DIR, "workspace-useful") ||
  ...
  describe("resolveWebChatDir", () => {
    it("returns <workspace>/.hermes/web-chat for active workspace (per-workspace chat isolation)", async () => {
      const {
        resolveWebChatDir,
        setUIActiveWorkspace,
        mockExists,
      ...
      expect(resolveWebChatDir()).toBe(join(wsDir, ".hermes", "web-chat"));
    });
    ...
    it("different workspaces produce different chat directories", async () => {
      ...
      expect(chatWork).toBe(join(workDir, ".hermes", "web-chat"));
      expect(chatPersonal).toBe(join(personalDir, ".hermes", "web-chat"));
    });
    ...
    it("falls back to root workspace path when no workspace is active", async () => {
      ...
      expect(resolveWebChatDir()).toBe(
        join(STATE_DIR, "workspace", ".hermes", "web-chat"),
      );
    });
  });
  ...
  describe("ensureManagedWorkspaceRouting", () => {
    it("repairs the default workspace agent and prunes chat slots without flipping the active default agent", async () => {
      const { ensureManagedWorkspaceRouting, mockExists, mockReadFile, mockWriteFile } =
        await importWorkspace();
      const configPath = join(STATE_DIR, "hermes.json");
  ...
  describe("registerWorkspacePath / getRegisteredWorkspacePath", () => {
    it("registerWorkspacePath is now a no-op (custom paths disabled)", async () => {
      ...
      const stateWrites = mockWriteFile.mock.calls.filter((c) =>
        (c[0] as string).includes(".useful-ui-state.json"),
      );
