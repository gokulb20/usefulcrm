...
vi.mock("./workspace", () => ({
	...
	resolveHermesStateDir: vi.fn(() => "/tmp/mock-state"),
...
describe("active-runs", () => {
	...
	describe("child process error handling", () => {
		...
		it("surfaces scope error with remediation steps when Gateway rejects operator.write", async () => {
			...
			const err = new Error("missing scope: operator.write. The Hermes Gateway rejected this request because the web app's credentials lack the required scope. Fix: run `npx usefulcrm bootstrap` to re-pair, or set OPENCLAW_GATEWAY_PASSWORD in the web app's environment.");
			child._emit("error", err);
