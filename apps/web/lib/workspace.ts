...
const UI_STATE_FILENAME = ".useful-ui-state.json";
const FIXED_STATE_DIRNAME = ".hermes-useful";
const WORKSPACE_PREFIX = "workspace-";
const ROOT_WORKSPACE_DIRNAME = "workspace";
const WORKSPACE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/i;
const DEFAULT_WORKSPACE_NAME = "default";
const DENCHCLAW_PROFILE = "useful";
...
function resolveHermesHomeDir(): string {
...
function stateDirPath(): string {
  return join(resolveHermesHomeDir(), FIXED_STATE_DIRNAME);
}
...
function uiStatePath(): string {
  return join(resolveHermesStateDir(), UI_STATE_FILENAME);
}
...
function workspaceNameFromPath(inputPath: string | null | undefined): string | null {
  ...
  const stateRoot = resolve(resolveHermesStateDir());
...
export function discoverWorkspaces(): DiscoveredWorkspace[] {
  ...
  for (const workspaceName of scanWorkspaceNames(stateDir)) {
    ...
    discovered.push({
      ...
      hasConfig: existsSync(join(stateDir, "hermes.json")),
    });
  }
...
export function registerWorkspacePath(_profile: string, _absolutePath: string): void {
  ...
  // ~/.hermes-useful/workspace (default) and ~/.hermes-useful/workspace-<name>.
}
...
// ---------------------------------------------------------------------------
// Hermes config (hermes.json) agent list helpers
// ---------------------------------------------------------------------------
...
function hermesConfigPath(): string {
  return join(resolveHermesStateDir(), "hermes.json");
}
...
function ensureConfigAgents(config: HermesConfig): void {
  if (!config.agents) {
    config.agents = {};
  }
}
...
function ensureConfigAgentList(config: HermesConfig): HermesAgentEntry[] {
  ensureConfigAgents(config);
...
function applyDefaultAgentMarker(list: HermesAgentEntry[], targetAgentId: string): boolean {
  let changed = false;
...
function removeChatSlotEntries(list: HermesAgentEntry[], baseId?: string): boolean {
 ...
 *
 * Chat-slot agents are no longer persisted in hermes.json. This function
...
export function resolveWebChatDir(): string {
  ...
  if (workspaceRoot) {
    return join(workspaceRoot, ".hermes", "web-chat");
  }

  const activeWorkspace = getActiveWorkspaceName();
  if (activeWorkspace) {
    return join(resolveWorkspaceDir(activeWorkspace), ".hermes", "web-chat");
  }

  // Fallback for first-run flows before any workspace is selected/created.
  return join(resolveWorkspaceDir(DEFAULT_WORKSPACE_NAME), ".hermes", "web-chat");
}

/** @deprecated Use `resolveWorkspaceRoot` instead. */
export const resolveUsefulRoot = resolveWorkspaceRoot;

/**
 * Return the workspace path prefix for the agent.
 * Returns the absolute workspace path (e.g. ~/.hermes/workspace),
 ...
 * e.g. for `~/.hermes/workspace/subdir/workspace.duckdb` returns `"subdir"`.
