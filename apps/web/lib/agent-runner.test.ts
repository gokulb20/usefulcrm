...
vi.mock("./workspace", () => ({
	...
	resolveHermesStateDir: () => "/tmp/__agent_runner_test_state",
}));
...
describe("agent-runner", () => {
	...
	describe("enhanceScopeError", () => {
		it("returns actionable message for 'missing scope: operator.write'", async () => {
			...
			expect(result).toContain("npx usefulcrm bootstrap");
	...
	describe("scope error handling", () => {
		it("emits enhanced error when connect fails with missing scope", async () => {
			...
			expect(stderr).toContain("npx usefulcrm bootstrap");
			proc.kill("SIGTERM");
		});
		...
		it("does not alter non-scope errors", async () => {
			...
			expect(stderr).not.toContain("npx usefulcrm bootstrap");
			proc.kill("SIGTERM");
		});
	});
