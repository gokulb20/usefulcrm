...
const NEW_STATE_DIRNAME = ".hermes-useful";
 ...
 * UsefulCRM always pins this to ~/.hermes-useful.
 */
 ...
 * Default: ~/.hermes/hermes.json (or $OPENCLAW_STATE_DIR/hermes.json)
 */
...
export function resolveDefaultConfigCandidates(
  ...
  const hermesStateDir = env.OPENCLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (hermesStateDir) {
    const resolved = resolveUserPath(hermesStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/hermes-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `hermes-${uid}` : "hermes";
