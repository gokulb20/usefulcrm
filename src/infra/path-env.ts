...
function candidateBinDirs(opts: EnsureHermesPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `hermes` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "hermes");
  ...
  if (allowProjectLocalBin) {
    ...
    if (isExecutable(path.join(localBinDir, "hermes"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `hermes` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureHermesCliOnPath(opts: EnsureHermesPathOpts = {}) {
