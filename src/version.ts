...
const CORE_PACKAGE_NAME = "usefulcrm";
...
// Single source of truth for the current UsefulCRM version.
...
let _cachedHermesVersion: string | undefined;

export function resolveHermesVersion(): string | undefined {
  if (_cachedHermesVersion !== undefined) return _cachedHermesVersion || undefined;

  const envVersion = (process.env.OPENCLAW_VERSION || process.env.OPENCLAW_SERVICE_VERSION)?.trim();
  if (envVersion) {
    _cachedHermesVersion = envVersion;
    return envVersion;
  }

  try {
    const req = createRequire(import.meta.url);
    const pkg = req("hermes/package.json") as { version?: string };
    const v = pkg.version?.trim();
    _cachedHermesVersion = v || "";
    return v || undefined;
  } catch {
    _cachedHermesVersion = "";
    return undefined;
  }
}
