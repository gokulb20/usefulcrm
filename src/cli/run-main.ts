...
import { ensureHermesCliOnPath } from "../infra/path-env.js";
...
export function rewriteBareArgvToBootstrap(
  ...
  if (resolveCliName(argv) !== "usefulcrm") {
    return argv;
  }
...
export function shouldDelegateToGlobalHermes(
  argv: string[],
  env: NodeJS.ProcessEnv = process.env,
): boolean {
...
async function delegateToGlobalHermes(argv: string[]): Promise<number> {
  if (
    isTruthyEnvValue(process.env.DENCHCLAW_DELEGATED) ||
    isTruthyEnvValue(process.env.OPENCLAW_DELEGATED)
  ) {
    throw new Error(
      "Hermes delegation loop detected. Check PATH so `hermes` resolves to the global Hermes CLI.",
    );
  }
  const delegatedArgv = argv.slice(2);
  return await new Promise<number>((resolve, reject) => {
    const child = spawn("hermes", delegatedArgv, {
      stdio: "inherit",
      env: {
        ...process.env,
    ...
    child.once("error", (error) => {
      ...
      if (err?.code === "ENOENT") {
        reject(
          new Error(
            [
              "Global `hermes` CLI was not found on PATH.",
              "Install it once with: npm install -g hermes",
            ].join("\n"),
          ),
        );
        return;
...
export async function runCli(argv: string[] = process.argv) {
  ...
  if (shouldEnsureCliPath(normalizedArgv)) {
    ensureHermesCliOnPath();
  }

  // Enforce the minimum supported runtime before doing any work.
  assertSupportedRuntime();

  // Show the animated UsefulCRM banner early so it appears for ALL invocations
  // (bare `usefulcrm`, subcommands, help, etc.). The bannerEmitted flag inside
  ...
  if (shouldDelegateToGlobalHermes(parseArgv)) {
    const exitCode = await delegateToGlobalHermes(parseArgv);
