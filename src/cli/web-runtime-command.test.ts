...
const workspaceSeedMocks = vi.hoisted(() => ({
  ...
  syncManagedSkills: vi.fn(() => ({
    ...
    workspaceDirs: ["/tmp/.hermes-useful/workspace"],
...
const webRuntimeMocks = vi.hoisted(() => ({
  ...
  resolveManagedWebRuntimeServerPath: vi.fn(() => "/tmp/.hermes-useful/web-runtime/app/server.js"),
  resolveHermesCommandOrThrow: vi.fn(() => "/usr/local/bin/hermes"),
  resolveProfileStateDir: vi.fn(() => "/tmp/.hermes-useful"),
  runHermesCommand: vi.fn(async () => ({ code: 0, stdout: '{"ok":true}', stderr: "" })),
  startManagedWebRuntime: vi.fn(() => ({
    started: true,
    pid: 7788,
    runtimeServerPath: "/tmp/.hermes-useful/web-runtime/app/server.js",
  })),
...
vi.mock("./web-runtime.js", () => ({
  ...
  resolveHermesCommandOrThrow: webRuntimeMocks.resolveHermesCommandOrThrow,
  resolveProfileStateDir: webRuntimeMocks.resolveProfileStateDir,
  runHermesCommand: webRuntimeMocks.runHermesCommand,
...
describe("updateWebRuntimeCommand", () => {
  beforeEach(() => {
    ...
    workspaceSeedMocks.discoverWorkspaceDirs.mockReturnValue(["/tmp/.hermes-useful/workspace"]);
  ...
  it("fails closed in non-interactive major upgrades without explicit approval (enforces mandatory operator consent)", async () => {
    ...
    ).rejects.toThrow("Major Useful upgrade detected");

    expect(spawnMock).not.toHaveBeenCalled();
    expect(webRuntimeMocks.ensureManagedWebRuntime).not.toHaveBeenCalled();
  });

  it("runs Hermes update before refreshing web runtime on major transitions (protects upgrade compatibility)", async () => {
    webRuntimeMocks.evaluateMajorVersionTransition.mockReturnValue({
      previousMajor: 2,
      currentMajor: 3,
      isMajorTransition: true,
  ...
  it("syncs managed skills during update and includes result in summary", async () => {
    ...
    expect(workspaceSeedMocks.discoverWorkspaceDirs).toHaveBeenCalledWith("/tmp/.hermes-useful");
  ...
  it("skips gateway daemon restart and LaunchAgent in daemonless mode", async () => {
    webRuntimeMocks.runHermesCommand.mockClear();
    ...
    expect(webRuntimeMocks.runHermesCommand).not.toHaveBeenCalled();
  ...
  it("skips Hermes update on minor upgrades while still refreshing runtime (avoids unnecessary blocking)", async () => {
    webRuntimeMocks.evaluateMajorVersionTransition.mockReturnValue({
      previousMajor: 2,
      currentMajor: 2,
      isMajorTransition: false,
...
describe("startWebRuntimeCommand", () => {
  ...
  it("fails closed when non-useful listeners still own the port (prevents cross-process takeover)", async () => {
    webRuntimeMocks.stopManagedWebRuntime.mockResolvedValue({
      port: 3100,
      stoppedPids: [],
    ...
    await expect(startWebRuntimeCommand({}, runtime)).rejects.toThrow("non-Useful listener");
  ...
  it("fails with actionable remediation when managed runtime is missing (requires explicit update/bootstrap)", async () => {
    ...
    await expect(startWebRuntimeCommand({}, runtime)).rejects.toThrow("npx usefulcrm update");
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("falls back to UsefulCRM port 19001 when manifest has no lastGatewayPort (prevents 18789 hijack)", async () => {
  ...
  it("falls back to UsefulCRM port 19001 when manifest is null (fresh install, prevents 18789 hijack)", async () => {
