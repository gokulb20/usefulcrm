...
| `profile.ts`            | root vs command-local bootstrap profile flag                             | `usefulcrm --profile X bootstrap` and `usefulcrm bootstrap --profile X` resolve to identical profile env | `src/cli/profile.test.ts` (existing, expand)                      |
