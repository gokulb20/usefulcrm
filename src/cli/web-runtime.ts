...
export function resolveProfileStateDir(
  ...
  return path.join(home, ".hermes-useful");
}
...
export function installManagedWebRuntime(params: {
  ...
  const manifest: ManagedWebRuntimeManifest = {
    schemaVersion: 1,
    deployedUsefulVersion: params.usefulVersion,
...
export async function ensureManagedWebRuntime(params: {
  ...
  const install = installManagedWebRuntime({
    ...
    usefulVersion: params.usefulVersion,
    webPort: params.port,
    gatewayPort: params.gatewayPort,
  });
  ...
  if (foreign.length > 0) {
    ...
    return {
      ready: false,
      reason: `port ${params.port} is owned by non-Useful process(es): ${detail}`,
    };
  }
...
export function resolveHermesCommandOrThrow(): string {
  try {
    const locator = process.platform === "win32" ? "where" : "which";
    const output = execFileSync(locator, ["hermes"], { encoding: "utf-8" }).trim();
    const first = output
      .split(/\r?\n/)
      .map((line) => line.trim())
      .find(Boolean);
    ...
    throw new Error(
      "Global `hermes` CLI was not found on PATH. Install it with: npm install -g hermes",
    );
  }
}

export async function runHermesCommand(params: {
