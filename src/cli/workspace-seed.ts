...
/**
 * Read hermes.json (or legacy config.json) and return all unique workspace
...
export function seedSampleApp(appsDir: string): void {
  const appDir = path.join(appsDir, "hello.useful.app");
  if (existsSync(appDir)) return;

  mkdirSync(appDir, { recursive: true });

  writeFileSync(
    path.join(appDir, ".useful.yaml"),
    `name: "Hello World"
description: "A sample UsefulCRM app"
icon: "sparkles"
version: "1.0.0"
entry: "index.html"
runtime: "static"
...
<body>
  ...
  <script>
    async function init() {
      ...
      try {
        const result = await window.useful.db.query("SELECT name, entry_count FROM objects");
        ...
        if (!result.rows || result.rows.length === 0) {
          el.textContent = 'No objects found yet. Create some in UsefulCRM!';
        }
      } catch (err) {
