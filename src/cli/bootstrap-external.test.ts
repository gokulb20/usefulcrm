...
function createTempStateDir(): string {
  const homeDir = path.join(
    tmpdir(),
    `usefulcrm-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  const stateDir = path.join(homeDir, ".hermes-useful");
...
describe("bootstrap-external diagnostics", () => {
  ...
  it("passes agent-auth for Useful Cloud when apiKey is stored on the custom provider config", () => {
    const dir = createTempStateDir();
    writeConfig(dir, {
      agents: { defaults: { model: { primary: "useful-cloud/anthropic.claude-opus-4-6-v1" } } },
      models: {
        providers: {
          "useful-cloud": {
            baseUrl: "https://gateway.merseoriginals.com/v1",
            apiKey: "useful_cfg_key_123",
          },
        },
      },
    });
  ...
  it("includes break-glass guidance only for device signature/token mismatch failures", () => {
    ...
    expect(String(gateway.remediation)).toContain("--profile useful");
  });
...
describe("checkAgentAuth", () => {
  ...
  it("returns ok when a custom provider apiKey exists in hermes.json", () => {
    writeConfig(stateDir, {
      models: {
        providers: {
          "useful-cloud": {
    ...
    const result = checkAgentAuth(stateDir, "useful-cloud");
    expect(result.ok).toBe(true);
...
describe("bootstrap-external rollout env helpers", () => {
  it("resolves rollout stage from usefulcrm/hermes env vars", () => {
...
describe("readExistingGatewayPort", () => {
  ...
  it("reads numeric port from hermes.json (normal config path)", () => {
    writeConfig(stateDir, { gateway: { port: 19001 } });
    expect(readExistingGatewayPort(stateDir)).toBe(19001);
  });

  it("falls back to config.json when hermes.json is absent (legacy config support)", () => {
    writeFileSync(
  ...
  it("prefers hermes.json over config.json when both exist (config precedence)", () => {
    writeConfig(stateDir, { gateway: { port: 19001 } });
    writeFileSync(
      path.join(stateDir, "config.json"),
  ...
  it("returns undefined for malformed JSON (handles corrupt config gracefully)", () => {
    writeFileSync(path.join(stateDir, "hermes.json"), "not valid json{{{");
    expect(readExistingGatewayPort(stateDir)).toBeUndefined();
  });
...
describe("isPersistedPortAcceptable", () => {
  ...
  it("rejects 18789 (prevents Hermes port hijack on launchd restart)", () => {
    expect(isPersistedPortAcceptable(18789)).toBe(false);
  });

  it("accepts UsefulCRM's own port range (normal operation)", () => {
  ...
  it("accepts valid 19001 from config (end-to-end: read + guard allows UsefulCRM port)", () => {
    writeConfig(stateDir, { gateway: { port: 19001 } });
    const port = readExistingGatewayPort(stateDir);
