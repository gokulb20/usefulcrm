...
function createWebProfilesResponse(params?: {
  ...
  const payload = params?.payload ?? { profiles: [], activeProfile: "useful" };
  return {
    status,
...
function createTempStateDir(): string {
  ...
  const dir = path.join(os.tmpdir(), `usefulcrm-bootstrap-${suffix}`);
...
function writeBootstrapFixtures(stateDir: string): void {
  ...
  writeFileSync(path.join(stateDir, "hermes.json"), JSON.stringify(config));
...
function applyConfigSet(stateDir: string, keyPath: string, rawValue: string): void {
  const configPath = path.join(stateDir, "hermes.json");
...
describe("bootstrapCommand always-onboard behavior", () => {
  ...
  beforeEach(() => {
    ...
    stateDir = path.join(homeDir, ".hermes-useful");
    ...
    promptMocks.textValue = "useful_test_key";
    ...
    spawnMock.mockImplementation((command, args = [], options) => {
      ...
      if (commandString === "hermes" && argList[0] === "--version") {
      ...
      if (
        ...
        return createMockChild({
          ...
          stdout: reportMissing
            ? '{"dependencies":{}}'
            : '{"dependencies":{"hermes":{"version":"2026.3.1"}}}',
        }) as never;
      }
      ...
      if ((commandString === "which" || commandString === "where") && argList[0] === "hermes") {
        return createMockChild({ code: 0, stdout: "/usr/local/bin/hermes\n" }) as never;
      }
      if (
        commandString === "hermes" &&
      ...
      if (commandString === "hermes" && argList.includes("onboard")) {
        if (driftGatewayModeAfterOnboard) {
          gatewayModeConfigValue = "remote\n";
        }
        return createMockChild({ code: 0, stdout: "ok\n" }) as never;
      }
      if (commandString === "hermes" && argList.includes("devices") && argList.includes("list")) {
        return createMockChild({
          code: 0,
          stdout: `${JSON.stringify({ pending: pendingDeviceRequests, paired: pairedDevices })}\n`,
        }) as never;
      }
      if (commandString === "hermes" && argList.includes("devices") && argList.includes("approve")) {
      ...
      if (commandString === "hermes" && argList.includes("health")) {
        healthCallCount += 1;
  ...
  it("runs onboard every bootstrap even when config already exists (prevents stale auth drift)", async () => {
    ...
    const onboardCalls = spawnCalls.filter(
      (call) => call.command === "hermes" && call.args.includes("onboard"),
    );
  ...
  it("stages gateway.mode=local in raw JSON before onboard so first daemon start does not drift (no CLI calls pre-profile)", async () => {
    ...
    const preOnboardGatewayModeCliSet = spawnCalls.findIndex((call, index) => {
      const onboardIndex = spawnCalls.findIndex(
        (c) => c.command === "hermes" && c.args.includes("onboard"),
      );
      return (
        index < onboardIndex &&
        call.command === "hermes" &&
  ...
  it("stages gateway.port in raw JSON before onboard so first daemon start uses UsefulCRM's port (no CLI calls pre-profile)", async () => {
    const runtime: RuntimeEnv = {
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
  ...
  it("applies gateway.port via CLI after onboard so onboarding defaults cannot desync UsefulCRM's gateway target", async () => {
    const runtime: RuntimeEnv = {
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
  ...
  it("ignores bootstrap --profile override and keeps useful profile (prevents profile drift)", async () => {
    const runtime: RuntimeEnv = {
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
    };
    process.env.OPENCLAW_PROFILE = "useful";
    ...
    expect(onboardCall?.args).toEqual(expect.arrayContaining(["--profile", "useful"]));
    expect(onboardCall?.args.includes("team-a")).toBe(false);
    expect(summary.profile).toBe("useful");
  });
  ...
  it("uses bootstrap-owned Useful Cloud setup and skips Hermes auth onboarding", async () => {
    writeFileSync(
      ...
      JSON.stringify({
        ...
        gateway: { mode: "local" },
        plugins: {
          allow: ["useful-cloud-provider"],
          load: {
            paths: [path.join(stateDir, "extensions", "useful-cloud-provider")],
          },
          entries: {
            "useful-cloud-provider": {
              enabled: true,
            },
          },
        },
    ...
    mkdirSync(path.join(stateDir, "extensions", "useful-cloud-provider"), { recursive: true });
    writeFileSync(
      path.join(stateDir, "extensions", "useful-cloud-provider", "index.ts"),
      "export {};\n",
    );
    fetchBehavior = async (url: string) => {
    ...
    await bootstrapCommand(
      {
        ...
        skipUpdate: true,
        usefulCloud: true,
        usefulCloudApiKey: "useful_live_key",
        usefulCloudModel: "anthropic.claude-opus-4-6-v1",
      },
      runtime,
    );
    ...
    const updatedConfig = JSON.parse(readFileSync(path.join(stateDir, "hermes.json"), "utf-8"));
    expect(updatedConfig.models.providers["useful-cloud"].apiKey).toBe("useful_live_key");
    expect(updatedConfig.agents.defaults.model.primary).toBe(
      "useful-cloud/anthropic.claude-opus-4-6-v1",
    );
    expect(updatedConfig.agents.defaults.models["useful-cloud/anthropic.claude-opus-4-6-v1"]).toEqual(
      expect.objectContaining({ alias: "Claude Opus 4.6 (Useful Cloud)" }),
    );
    expect(updatedConfig.plugins.allow).toContain("posthog-analytics");
    expect(updatedConfig.plugins.allow).toContain("useful-ai-gateway");
    expect(updatedConfig.plugins.allow).not.toContain("useful-cloud-provider");
    expect(updatedConfig.plugins.entries["useful-cloud-provider"]).toBeUndefined();
    expect(updatedConfig.plugins.entries["useful-ai-gateway"]).toEqual(
    ...
    expect(updatedConfig.plugins.installs["useful-ai-gateway"]).toEqual(
      expect.objectContaining({
        source: "path",
        installPath: expect.stringContaining(path.join("extensions", "useful-ai-gateway")),
      }),
    );
    expect(existsSync(path.join(stateDir, "extensions", "useful-cloud-provider"))).toBe(false);
  });

  it("falls back to UsefulCRM's bundled model list when the public gateway catalog is unavailable", async () => {
    fetchBehavior = async (url: string) => {
      if (url.includes("gateway.merseoriginals.com/v1/models")) {
        return createJsonResponse({ status: 200, payload: { object: "list", data: [] } });
      }
    ...
    promptMocks.textValue = "useful_retry_key";
    ...
    const updatedConfig = JSON.parse(readFileSync(path.join(stateDir, "hermes.json"), "utf-8"));
    expect(updatedConfig.agents.defaults.model.primary).toBe(
      "useful-cloud/anthropic.claude-sonnet-4-6-v1",
    );
    expect(updatedConfig.models.providers["useful-cloud"].models).toEqual(
      expect.arrayContaining([
  ...
  it("re-prompts for Useful Cloud every bootstrap and pre-fills the saved key and model", async () => {
    writeFileSync(
      path.join(stateDir, "hermes.json"),
      JSON.stringify({
        agents: {
          defaults: {
            model: { primary: "useful-cloud/anthropic.claude-opus-4-6-v1" },
          },
        },
        models: {
          providers: {
    ...
    promptMocks.textValue = "useful_saved_key";
  ...
  it("runs update before onboarding when --update-now is set", async () => {
    ...
    const updateIndex = spawnCalls.findIndex(
      (call) =>
        call.command === "hermes" && call.args.includes("update") && call.args.includes("--yes"),
    );
    const onboardIndex = spawnCalls.findIndex(
      (call) => call.command === "hermes" && call.args.includes("onboard"),
    );
  ...
  it("skips update prompt right after installing hermes@latest (avoids redundant update checks)", async () => {
    ...
    const installedGlobalHermes = spawnCalls.some(
      (call) =>
        call.command === "npm" &&
        call.args.includes("install") &&
        call.args.includes("-g") &&
        call.args.includes("hermes@latest"),
    );
    ...
    expect(installedGlobalHermes).toBe(true);
    expect(promptMocks.confirm).toHaveBeenCalledTimes(1);
  ...
  it("reuses recent Hermes CLI availability checks to avoid repeated npm/which probes", async () => {
    const runtime: RuntimeEnv = {
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
    ...
    const firstProbeCounts = {
      ...
      shellWhich: spawnCalls.filter(
        (call) =>
          (call.command === "which" || call.command === "where") && call.args[0] === "hermes",
      ).length,
      versionCheck: spawnCalls.filter(
        (call) => call.command === "hermes" && call.args[0] === "--version",
      ).length,
    };
  ...
  it("pins workspace config to default workspace path during bootstrap", async () => {
    ...
    expect(lastArgs).toEqual(
      expect.arrayContaining(["--profile", "useful", "config", "set", "agents.defaults.workspace"]),
    );
    const configuredWorkspace = lastArgs.at(-1) ?? "";
    expect(configuredWorkspace).toContain(path.join(".hermes-useful", "workspace"));
    expect(configuredWorkspace).not.toContain("workspace-useful");
  });
  ...
  it("forces tools.profile to full during bootstrap (prevents messaging-only tool drift)", async () => {
    ...
    expect(lastArgs).toEqual(
      expect.arrayContaining(["--profile", "useful", "config", "set", "tools.profile", "full"]),
    );
  ...
  it("does not call gateway install/start fallback when onboarding is always used", async () => {
    ...
    const gatewayStartCalled = spawnCalls.some(
      (call) =>
        call.command === "hermes" && call.args.includes("gateway") && call.args.includes("start"),
    );
  ...
  it("installs global Hermes even when a local binary already resolves", async () => {
    forceGlobalMissing = true;
    const runtime: RuntimeEnv = {
      log: vi.fn(),
      error: vi.fn(),
    ...
    expect(summary.installedHermesCli).toBe(true);
  });
  ...
  it("recovers without autofix when gateway just needs a moment after restart (no false gateway-closed)", async () => {
    ...
    const doctorFixCalled = spawnCalls.some(
      (call) =>
        call.command === "hermes" && call.args.includes("doctor") && call.args.includes("--fix"),
    );
  ...
  it("runs doctor/gateway autofix steps only after all retried probes fail", async () => {
    ...
    const gatewayStopCalled = spawnCalls.some(
      (call) =>
        call.command === "hermes" && call.args.includes("gateway") && call.args.includes("stop"),
    );
    const gatewayInstallCalled = spawnCalls.some(
      (call) =>
        call.command === "hermes" &&
    ...
    const gatewayRestartCalledInAutofix = spawnCalls.some(
      (call) =>
        call.command === "hermes" && call.args.includes("gateway") && call.args.includes("restart"),
    );
  ...
  it("keeps preferred web port and does not probe sibling ports", async () => {
    ...
    fetchBehavior = async (url: string) => {
      if (url.includes("127.0.0.1:3100/api/profiles")) {
        ...
        return createWebProfilesResponse({
          status: 200,
          payload: { profiles: [], activeProfile: "useful" },
        });
      }
  ...
  it("applies exec and elevated commands via CLI after onboard (prevents onboard wizard drift)", async () => {
    ...
    for (const { key, value } of elevatedSettings) {
      ...
      expect(postOnboardSetCall?.args).toEqual(
        expect.arrayContaining(["--profile", "useful", "config", "set", key, value]),
      );
    }
  });
  ...
  it("reapplies elevated commands on repeated bootstrap runs (idempotent safety)", async () => {
    ...
    for (const call of elevatedEnabledCalls) {
      expect(call.args).toEqual(
        expect.arrayContaining(["--profile", "useful", "config", "set", "tools.elevated.enabled", "true"]),
      );
    }
  });

  it("preserves exec and elevated config in final hermes.json after full bootstrap cycle", async () => {
    const runtime: RuntimeEnv = {
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
  ...
  it("strips npm_config_* env vars from npm global commands (prevents npx prefix hijack)", async () => {
    ...
    process.env.npm_package_name = "usefulcrm";
