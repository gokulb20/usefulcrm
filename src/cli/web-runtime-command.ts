...
async function runHermesUpdateWithProgress(hermesCommand: string): Promise<void> {
  const s = spinner();
  s.start("Updating Hermes (required for this major Useful upgrade)...");
  const result = await new Promise<SpawnResult>((resolve, reject) => {
    const child = spawn(hermesCommand, ["update", "--yes"], {
  ...
  if (result.code === 0) {
    s.stop("Hermes update complete.");
    return;
  }
  const detail = firstNonEmptyLine(result.stderr, result.stdout);
  s.stop(detail ? `Hermes update failed: ${detail}` : "Hermes update failed.");
  throw new Error(
    detail
      ? `Hermes update failed.\n${detail}`
      : "Hermes update failed. Fix this before running `npx usefulcrm update` again.",
  );
}
...
async function ensureMajorUpgradeAcknowledged(params: {
  ...
  if (params.nonInteractive || !process.stdin.isTTY) {
    if (!params.yes) {
      throw new Error(
        `Major Useful upgrade detected (${params.previousVersion ?? "unknown"} -> ${params.currentVersion}). Re-run with --yes to approve the required Hermes update.`,
      );
    }
    return;
  }
  ...
  const decision = await confirm({
    message: stylePromptMessage(
      `Major Useful upgrade detected (${params.previousVersion ?? "unknown"} -> ${params.currentVersion}). Continue with mandatory Hermes update now?`,
    ),
    initialValue: true,
  });
  if (isCancel(decision) || !decision) {
    params.runtime.log(
      theme.warn("Update cancelled. Hermes update is required for major upgrades."),
    );
    throw new Error("Major upgrade requires Hermes update approval.");
  }
}
...
async function restartGatewayDaemon(params: {
  ...
  try {
    hermesCommand = resolveHermesCommandOrThrow();
  } catch {
    return { restarted: false, error: "hermes CLI not found on PATH" };
  }
  ...
  const restartResult = await runHermesCommand({
    hermesCommand,
...
export async function updateWebRuntimeCommand(
  ...
  const transition = evaluateMajorVersionTransition({
    previousVersion: previousManifest?.deployedUsefulVersion,
  ...
  if (transition.isMajorTransition) {
    const hermesCommand = resolveHermesCommandOrThrow();
    await runHermesUpdateWithProgress(hermesCommand);
  }
  ...
  if (!opts.json) {
    runtime.log("");
    runtime.log(theme.heading("Useful web update"));
    ...
    if (summary.skippedForeignPids.length > 0) {
      runtime.log(
        theme.warn(
          `Skipped non-Useful listeners on ${selectedPort}: ${summary.skippedForeignPids.join(", ")}`,
        ),
      );
    }
...
export async function stopWebRuntimeCommand(
  ...
  runtime.log("");
  runtime.log(theme.heading("Useful web stop"));
  runtime.log(`Profile: ${profile}`);
  runtime.log(`Web port: ${selectedPort}`);
  runtime.log(
  ...
  if (summary.skippedForeignPids.length > 0) {
    runtime.log(
      theme.warn(
        `Left non-Useful listener(s) running on ${selectedPort}: ${summary.skippedForeignPids.join(", ")}`,
      ),
    );
  }
  return summary;
...
export async function startWebRuntimeCommand(
  ...
  if (stopResult.skippedForeignPids.length > 0) {
    throw new Error(
      `Cannot start on ${selectedPort}; non-Useful listener(s) still own the port: ${stopResult.skippedForeignPids.join(", ")}`,
    );
  }
  ...
  if (!startResult.started) {
    ...
    throw new Error(
      [
        ...
        "Run `npx usefulcrm update` (or `npx usefulcrm`) to install/update the web runtime first.",
      ].join(" "),
    );
  }
  ...
  runtime.log("");
  runtime.log(theme.heading(`Useful web ${label}`));
  runtime.log(`Profile: ${profile}`);
  runtime.log(`Web port: ${selectedPort}`);
  if (daemonless) {
    runtime.log(`Gateway: skipped (daemonless mode)`);
...
export async function verifyHermesHealthForUpdate(profile: string): Promise<void> {
  const hermesCommand = resolveHermesCommandOrThrow();
