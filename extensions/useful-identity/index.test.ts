...
describe("buildIdentityPrompt", () => {
  ...
  it("includes chat history path so agent can reference past conversations", () => {
    ...
    expect(prompt).toContain(".hermes/web-chat/");
    expect(prompt).toContain(
      path.join(workspaceDir, ".hermes/web-chat/"),
    );
  });
  ...
  it("references UsefulCRM branding, not Hermes (prevents identity confusion)", () => {
    const prompt = buildIdentityPrompt(workspaceDir);
    expect(prompt).toContain("You are **UsefulCRM**");
    expect(prompt).toContain("always use **UsefulCRM** (not Hermes)");
  });
});
...
describe("register", () => {
  ...
  it("does not register handler when plugin is explicitly disabled (respects config)", () => {
    const api = {
      config: { plugins: { entries: { "useful-identity": { config: { enabled: false } } } } },
      on: vi.fn(),
    };
    register(api);
  ...
  it("handler returns system context when workspace is configured", () => {
    ...
    expect(result).toEqual({
      prependSystemContext: expect.stringContaining("UsefulCRM"),
    });
  });
