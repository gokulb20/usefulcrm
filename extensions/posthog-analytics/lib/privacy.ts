...
function resolveConfigPath(hermesConfig?: any): string {
  const stateDir =
    hermesConfig?.stateDir ??
    join(process.env.HOME || homedir(), ".hermes-useful");
  return join(stateDir, "telemetry.json");
}

/**
 * Read privacy mode from UsefulCRM's telemetry config.
 * Default is true (privacy on) when the file is missing or unreadable.
 */
export function readPrivacyMode(hermesConfig?: any): boolean {
  try {
    const configPath = resolveConfigPath(hermesConfig);
 ...
 */
export function readPersonInfo(hermesConfig?: any): PersonInfo | null {
  ...
  try {
    ...
    if (typeof raw.usefulOrgId === "string" && raw.usefulOrgId) info.usefulOrgId = raw.usefulOrgId;
 ...
 */
export function readOrCreateAnonymousId(hermesConfig?: any): string {
