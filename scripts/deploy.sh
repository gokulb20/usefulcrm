#!/usr/bin/env bash
# deploy.sh — build and publish usefulcrm to npm
#
...
run_npx_smoke_checks() {
  ...
  verify_npx_command "$version" "npx usefulcrm" \
    npx --yes "${PACKAGE_NAME}@${version}" --version
  verify_npx_invocation "npx usefulcrm update --help" \
    npx --yes "${PACKAGE_NAME}@${version}" update --help
  verify_npx_invocation "npx usefulcrm start --help" \
    npx --yes "${PACKAGE_NAME}@${version}" start --help
  verify_npx_invocation "npx usefulcrm stop --help" \
...
# by default, but we want `npm i -g usefulcrm` to always resolve to
# the most recently published version. The root package already exposes both
# `usefulcrm` and `useful` binaries, so there is no separate alias package.
...
echo "commands: ${PACKAGE_NAME}, useful"
